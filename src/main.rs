...
fn http_read_request(stream: &mut TcpStream) -> Result<HttpRequest> {
  ...
  let string = String::from_utf8(receive)?.replace("\r\n", "\n");
...
fn parse_stream_request(path: &String) -> Result<Option<Screen>> {
  lazy_static! {
...
fn spawn_thread<F, T>(handler: F) -> JoinHandle<()>
where
  F: FnOnce() -> T,
  F: Send + 'static,
{
...
fn main() -> Result<()> {
  spawn_thread(http_server).join().unwrap();
  Ok(())
}
